...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (it_anim->second.active_rect_alpha > 25)
        it_anim->second.active_rect_alpha = 25;
    ...
    if (it_anim->second.active_outline_anim >= 20 && it_anim->second.active_rect_alpha >= 25)
        it_anim->second.clicked = false;


    window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(ImGuiCol_Button), 3.f);
...
bool ImGui::ButtonExEm(const char* label, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Change text color based on hover
    ImU32 text_color = GetColorU32(hovered ? ImGuiCol_TextHovered : ImGuiCol_Text);

    // Draw the text with the modified color
    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(text_color)));
    RenderTextClipped(rect.Min + ImVec2(0, 1), rect.Max + ImVec2(0, 1), label, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
...
bool ImGui::ButtonExx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool is_picked)
{
        struct checbox_anim
    {
        int active_outline_anim;
    ...
    float padding = 5.0f; // Expand rectangle size

    // Expand the rectangle for better visuals
    ImVec2 expandedMin = rect.Min - ImVec2(padding, padding);
    ImVec2 expandedMax = rect.Max + ImVec2(padding, padding);

    // Draw the rectangle layers
    window->DrawList->AddRectFilled(expandedMin, expandedMax, ImColor(11, 11, 11, 255), 3.f);
    window->DrawList->AddRectFilled(expandedMin, expandedMax, ImColor(77, 77, 77, it_anim->second.active_rect_alpha), 3.f);
    window->DrawList->AddRect(expandedMin, expandedMax, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);

    // Calculate center position for the text (Font Awesome icon)
    ImVec2 textPos = rect.Min + (rect.Max - rect.Min) * 0.5f - label_size * 0.5f; // Center text
    ...
    // Render centered text/icon
    RenderTextClipped(textPos, textPos + label_size, label, NULL, &label_size, ImVec2(0.5f, 0.5f));
...
bool ImGui::ButtonNew(const char* label, const ImVec2& size_arg, bool is_picked)
{
    return ButtonExx(label, size_arg, ImGuiButtonFlags_None, is_picked);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = GetFrameHeight() * 0.5f;
    ...
    if (label_size.x > 0.0f) {
        float text_offset_y = (square_sz - label_size.y) * 0.5f; // Center align text with checkbox
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + text_offset_y), label);
    }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (v)
        it_anim->second.active_rect_alpha += 10 * (0.5f - ImGui::GetIO().DeltaTime);
